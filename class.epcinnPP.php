...
public function variable_SUBETUFACTURA(){
	...
	private function validarFacturaXmlAdjunta($idPago = ''){
		$conn = $this->db();
		$session = isset($_SESSION['idCG']) ? $_SESSION['idCG'] : '';
		if($session == ''){
			return array('ok' => false, 'mensaje' => 'NO HAY UN PROVEEDOR SELECCIONADO');
		}

		$idPago = trim((string)$idPago);
		$filtroTemporal = "idTemporal = 'si'";
		if($idPago !== ''){
			$filtroTemporal = "(idTemporal = 'si' OR idTemporal = '".mysqli_real_escape_string($conn, $idPago)."')";
		}

		$variablequery = "select ADJUNTAR_FACTURA_XML from 07COMPROBACIONDOCT where idRelacion = '".$session."' and ".$filtroTemporal." and ADJUNTAR_FACTURA_XML is not null and ADJUNTAR_FACTURA_XML <> '' order by id desc limit 1";
		$arrayquery = mysqli_query($conn,$variablequery);
		$row = mysqli_fetch_array($arrayquery, MYSQLI_ASSOC);

		if(!isset($row['ADJUNTAR_FACTURA_XML']) || $row['ADJUNTAR_FACTURA_XML'] == ''){
			return array('ok' => false, 'mensaje' => 'NO SE PUEDE GUARDAR: PRIMERO DEBES SUBIR Y GUARDAR EL ARCHIVO XML.');
		}

		$rutaXml = __ROOT3__.'/includes/archivos/'.$row['ADJUNTAR_FACTURA_XML'];
		if(!file_exists($rutaXml)){
			return array('ok' => false, 'mensaje' => 'NO SE PUEDE GUARDAR: EL ARCHIVO XML NO EXISTE EN EL SERVIDOR. VUELVE A SUBIRLO.');
		}

		return array('ok' => true, 'archivo' => $row['ADJUNTAR_FACTURA_XML']);
	}
